...
# server_url = "http://127.0.0.1:8000/api/"  # 替换为你的 ComfyUI 服务器地址
...
def download_by_name(filename, folder='', output='output'):
    rand = random.random()
    # 构造完整的 URL
    url = f"{server_url}/view?filename={filename}&subfolder={folder}&type={output}&rand={rand}"
    try:
        print('download-link:', url)
        # 发送请求
        response = requests.get(url)
...
def submit_image_task(prompt_text='a pretty girl face to the camera with smile'):
    # 检查调度队列是否为空
    p_id = query_running_queue()
    if not p_id == 'idle':
        print('服务正忙,调度失败')
        return
    prompt_id = call_comfyui_workflow(prompt_text)
    if prompt_id == 'error':
        print('服务异常,调度失败')
        return
    result = check_task(prompt_id)


def call_create_image(prompt_text='a pretty girl face to the camera with smile',level = 'middle', block=True, callback=None):
    p_id = query_running_queue()
    if not p_id == 'idle':
        print('服务正忙,调度失败')
        if callback:
